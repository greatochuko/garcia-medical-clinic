...
export default function PlanList({ auth, plans }) {
    const [selectedMenu, setSelectedMenu] = useState('Plan List');
    const [activeActionRow, setActiveActionRow] = useState(null);
    const [showAddModal, setShowAddModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [editingPlan, setEditingPlan] = useState(null);
    const [perPage, setPerPage] = useState(plans?.per_page || 10);
    const [sortDirection, setSortDirection] = useState(null);
    const [sortedPlans, setSortedPlans] = useState(plans?.data || []);
    ...
    // Sort plans when direction changes
    useEffect(() => {
        if (sortDirection === null) {
            setSortedPlans(plans?.data || []);
            return;
        }
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        put(route('plan-list.update', editingPlan.id), {
            onSuccess: () => {
    ...
    const handleDeleteConfirm = () => {
        if (itemToDelete) {
            router.delete(route('plan-list.destroy', itemToDelete.id), {
                onSuccess: () => {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-[14px] text-gray-800 leading-tight font-poppins">Plan List</h2>}
        >
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        ...
                                        <div className="grid grid-cols-[90%,10%] gap-4 px-6 sticky top-0 z-10" style={{
                                            ...
                                            <div className="font-semibold text-[14px]" style={{ color: '#666666' }}>
                                                PLAN NAME <SortArrow column="name" />
                                            </div>
                                        ...
                                        <div className="flex-1">
                                            {Array.isArray(sortedPlans) ? sortedPlans.map((plan, index) => (
                                                <div 
                                                    key={plan.id}
                                                    className="grid grid-cols-[90%,10%] gap-4 px-6 text-[14px] active:bg-transparent focus:outline-none"
                                                    style={{
                                                        height: '54px',
                                    ...
                                    <div className="relative flex items-center mt-4 px-4 py-4">
                                        ...
                                        <div className="w-full flex justify-center">
                                            <div className="flex items-center space-x-2">
                                                <button
                                                    onClick={() => handlePaginationClick(plans?.prev_page_url)}
                                                    disabled={!plans?.prev_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!plans?.prev_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                ...
                                                <span className="custom-text-color flex items-center px-1">
                                                    Page {plans?.current_page || 1} of {plans?.last_page || 1}
                                                </span>

                                                <button
                                                    onClick={() => handlePaginationClick(plans?.next_page_url)}
                                                    disabled={!plans?.next_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!plans?.next_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                    <div className="custom-text-color flex items-center">
                                                        <span className="mx-1">|</span>
                                                        <span>Next</span>
            ...
            <Modal show={showAddModal} onClose={() => setShowAddModal(false)} maxWidth="lg">
                <div className="template-modal-content">
                    <button
                        ...
                        className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 plan-list-close-btn active:bg-transparent focus:outline-none"
                        aria-label="Close"
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <h1 className="plan-list-modal-title mb-4 text-left">
                        ADD PLAN
                    </h1>
                    <form onSubmit={handleSubmit}>
                        ...
                        {/* <div className="mb-4">
                            <label htmlFor="description" className="block font-poppins font-medium text-[16px] text-left mb-2 template-label">
                                Description
                            </label>
                            <textarea
                                id="description"
            ...
            <Modal show={showEditModal} onClose={() => {
                ...
                <div className="template-modal-content">
                    <button
                        ...
                        className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 active:bg-transparent focus:outline-nones"
                        aria-label="Close"
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M18 6L6 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
