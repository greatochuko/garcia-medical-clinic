...
export default function FrequencyList({ auth, frequencies }) {
    const [selectedMenu, setSelectedMenu] = useState('Frequency List');
    const [activeActionRow, setActiveActionRow] = useState(null);
    const [showAddModal, setShowAddModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [editingFrequency, setEditingFrequency] = useState(null);
    const [perPage, setPerPage] = useState(frequencies.per_page || 10);
    const [sortDirection, setSortDirection] = useState(null);
    const [sortedFrequencies, setSortedFrequencies] = useState(frequencies.data);
    ...
    // Sort frequencies when direction changes
    useEffect(() => {
        if (sortDirection === null) {
            setSortedFrequencies(frequencies.data);
            return;
        }
        
        const sorted = [...frequencies.data].sort((a, b) => {
    ...
    }, [sortDirection, frequencies.data]);
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        put(route('frequency-list.update', editingFrequency.id), {
            onSuccess: () => {
    ...
    const handlePerPageChange = (e) => {
        ...
        router.get(route('frequency-list'), { 
            perPage: newPerPage,
            page: 1
        }, {
            preserveState: true,
    ...
    const handleDeleteClick = (frequency) => {
        setItemToDelete(frequency);
    ...
    const handleDeleteConfirm = () => {
        if (itemToDelete) {
            router.delete(route('frequency-list.destroy', itemToDelete.id), {
                onSuccess: () => {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold page-header text-gray-800 leading-tight font-poppins">Frequency List</h2>}
        >
            <Head title="Frequency List">
                            ...
                            <div className="flex-1 p-4 bg-white flex flex-col">
                                <div className="flex flex-col mb-4">
                                    <div className="w-full flex justify-center mb-4">
                                        <h3 className="font-poppins font-bold page-title text-[#429ABF]">
                                            FREQUENCY LIST
                                        </h3>
                                    </div>
                                    <div className="w-full flex justify-end">
                                        <button 
                                            className="text-white px-4 py-2 rounded-[6px] flex items-center save-button text-[14px] add-templates-btn"
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        ...
                                        <div className="grid grid-cols-[90%,10%] gap-4 px-6 sticky top-0 z-10" style={{
                                            ...
                                            <div className="font-semibold text-[14px]" style={{ color: '#666666' }}>
                                                FREQUENCY NAME <SortArrow column="name" />
                                            </div>
                                        ...
                                        <div className="flex-1">
                                            {Array.isArray(sortedFrequencies) ? sortedFrequencies.map((frequency, index) => (
                                                <div 
                                                    key={frequency.id}
                                            ...
                                            )) : <div className="p-4 text-center text-gray-500">No frequencies found</div>}
                                        </div>
                                    </div>
                                    </div>
<hr />
                                    ...
                                    <div className="relative flex items-center mt-4 px-4 py-4">
                                        ...
                                        <div className="w-full flex justify-center">
                                            <div className="flex items-center space-x-2">
                                                <button
                                                    onClick={() => handlePaginationClick(frequencies.prev_page_url)}
                                                    disabled={!frequencies.prev_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!frequencies.prev_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                    <div className="custom-text-color flex items-center">
                                                        <span className="pagination-arrow-left">&laquo;</span>
                                                        <span>Prev</span>
                                                ...
                                                <span className="custom-text-color flex items-center px-1">
                                                    Page {frequencies.current_page} of {frequencies.last_page}
                                                </span>

                                                <button
                                                    onClick={() => handlePaginationClick(frequencies.next_page_url)}
                                                    disabled={!frequencies.next_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!frequencies.next_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                    <div className="custom-text-color flex items-center">
                                                        <span className="pagination-divider">|</span>
                                                        <span>Next</span>
                                                        <span className="pagination-arrow-right">&raquo;</span>
                                                    </div>
                                                </button>
                                            </div>
                                        </div>
            ...
            <Modal show={showAddModal} onClose={() => setShowAddModal(false)} maxWidth="lg">
                <div className="template-modal-content">
                    <button
                        ...
                        className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 modal-close-button active:bg-transparent focus:outline-none"
                        aria-label="Close"
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
