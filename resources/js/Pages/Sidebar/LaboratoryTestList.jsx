...
import '../../../css/laboratory-test-list.css';

export default function LaboratoryTestList({ auth, tests }) {
    const [selectedMenu, setSelectedMenu] = useState('Laboratory Test List');
    const [activeActionRow, setActiveActionRow] = useState(null);
    const [showAddModal, setShowAddModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [editingTest, setEditingTest] = useState(null);
    const [perPage, setPerPage] = useState(tests?.per_page || 10);
    const [sortDirection, setSortDirection] = useState(null);
    const [sortedTests, setSortedTests] = useState(tests?.data || []);
    ...
    // Sort tests when direction changes
    useEffect(() => {
        if (sortDirection === null) {
            setSortedTests(tests?.data || []);
            return;
        }
        
        const sorted = [...(tests?.data || [])].sort((a, b) => {
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        put(route('laboratory-test.update', editingTest.id), {
            onSuccess: () => {
    ...
    const handlePerPageChange = (e) => {
        ...
        router.get(route('laboratory-test'), { 
            perPage: newPerPage,
            page: 1
        }, {
            preserveState: true,
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (!e.target.closest('.lab-test-action-menu')) {
                setActiveActionRow(null);
            }
        };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('laboratory-test.store'), {
            onSuccess: () => {
    ...
    const handleDeleteConfirm = () => {
        if (itemToDelete) {
            router.delete(route('laboratory-test.destroy', itemToDelete.id));
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-[14px] text-gray-800 leading-tight font-poppins">Laboratory Test List</h2>}
        >
            <Head title="Laboratory Test List">
                            ...
                            <div className="flex-1 p-4 bg-white flex flex-col">
                                <div className="flex flex-col mb-4">
                                    <div className="w-full flex justify-center mb-4">
                                        <h3 className="lab-test-title">
                                            LABORATORY TEST LIST
                                        </h3>
                                    </div>
                                    <div className="w-full flex justify-end">
                                        <button 
                                            className="text-white px-4 py-2 rounded-[6px] flex items-center save-button text-[14px] lab-test-add-btn"
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        ...
                                        <div className="grid grid-cols-[90%,10%] gap-4 px-6 sticky top-0 z-10" style={{
                                            ...
                                            <div className="font-semibold text-[14px]" style={{ color: '#666666' }}>
                                                NAME OF TESTS <SortArrow column="name" />
                                            </div>
                                        ...
                                        <div className="flex-1">
                                            {Array.isArray(sortedTests) ? sortedTests.map((test, index) => (
                                                <div 
                                                    key={test.id}
                                                    ...
                                                    <div className="flex justify-center">
                                                        <div className="relative action-dropdown">
                                                            <button 
                                                                ...
                                                                className="lab-test-action-btn active:bg-transparent focus:outline-none"
                                                            >
...
<hr />
                                    ...
                                    <div className="relative flex items-center mt-4 px-4 py-4">
                                        ...
                                        <div className="w-full flex justify-center">
                                            <div className="flex items-center space-x-2">
                                                <button
                                                    onClick={() => handlePaginationClick(tests?.prev_page_url)}
                                                    disabled={!tests?.prev_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!tests?.prev_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                    <div className="lab-test-text flex items-center">
                                                        <span className='font-light-blue' style={{ fontSize: '20px', marginRight: '2px' }}>&laquo;</span>
                                                        <span className="font-light-blue">Prev</span>
                                                        <span className="mx-1 font-light-blue">|</span>
                                                    </div>
                                                </button>

                                                <span className="lab-test-text-custome flex items-center px-1 font-light-blue">
                                                    Page {tests?.current_page || 1} of {tests?.last_page || 1}
                                                </span>

                                                <button
                                                    onClick={() => handlePaginationClick(tests?.next_page_url)}
                                                    disabled={!tests?.next_page_url}
                                                    className={`px-2 py-1 rounded flex items-center ${!tests?.next_page_url ? 'text-gray-400 cursor-not-allowed' : 'text-blue-600 hover:bg-blue-100'}`}
                                                >
                                                    <div className="lab-test-text flex items-center">
                                                        <span className="mx-1 font-light-blue">|</span>
                                                        <span className="font-light-blue">Next</span>
                                                        <span className='font-light-blue' style={{ fontSize: '20px', marginLeft: '2px' }}>&raquo;</span>
                                                    </div>
                                                </button>
                                            </div>
                                        </div>
            ...
            <Modal show={showAddModal} onClose={() => setShowAddModal(false)} maxWidth="lg">
                <div className="template-modal-content">
                    ...
                    <h1 className="lab-test-modal-title mb-4 text-left">
                        ADD TEST
                    </h1>
                    <form onSubmit={handleSubmit}>
                        <div className="mb-4">
                            <label htmlFor="name" className="block font-poppins font-medium text-[16px] text-left mb-2 lab-test-text">
            ...
            <Modal show={showEditModal} onClose={() => {
                ...
                <div className="template-modal-content">
                    <button
                        ...
                        className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 lab-test-close-btn active:bg-transparent focus:outline-none"
                        aria-label="Close"
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M18 6L6 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    ...
                    <form onSubmit={handleEditSubmit}>
                        <div className="mb-4">
                            <label htmlFor="edit-name" className="block font-poppins font-medium text-[16px] text-left mb-2 lab-test-text">
                                Test Name <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
