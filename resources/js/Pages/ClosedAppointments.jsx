...
export default function ClosedAppointments({ auth, patient }) {
    ...
    const SortArrow = ({ column }) => {
        return (
            <button
                onClick={handleSort}
                className="relative ml-1 inline-flex cursor-pointer touch-none select-none flex-col focus:outline-none active:bg-transparent"
                style={{
                    width: "12px",
                    height: "16px",
    ...
    const handleDeleteConfirm = () => {
        if (itemToDelete) {
            router.delete(
                route("appointments.destroy.closed", itemToDelete.id),
                {
                    onSuccess: () => {
        ...
        >
            ...
            <div className="py-3">
                <div className="max-w-8xl main-screen mx-8 bg-white">
                    ...
                    <br />
                    <div className="mb-7 mt-2 flex justify-between px-4">
                        <div className="flex w-full max-w-md items-center overflow-hidden rounded border border-gray-300">
                            ...
                            <button
                                className="flex h-full items-center justify-center rounded bg-[#429ABF] p-2 pl-3 pr-4"
                                onClick={() =>
                                    handleSearch({
                        ...
                        <table className="w-full">
                            <thead className="sticky top-0 z-10 bg-white">
                                ...
                                >
                                    ...
                                    <th className="px-4 py-3 text-left text-center">
                                        STATUS
                                    </th>
                                    ...
                                    >
                                        <td className="px-4 py-3">
                                            <div className="text-[32px]">
                                                <span
                                                    className={`font-bold ${item.queue_number.startsWith("R") ? "text-[#1D7498]" : "text-[#FF8000]"}`}
                                                >
                                                    {item.queue_number}
                                                </span>
                                            </div>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="font-light-blue font-semibold">
                                                {item.name}
                                            </div>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <div className="text-sm text-gray-500">
                                                <span className="rounded bg-[#EAEAEA] px-4 py-1 capitalize text-[#BBC1C7]">
                                                    {item.status
                                                        .replace(/_/g, " ")
                                                        .replace(
                                                            /\b\w/g,
                                        ...
                                        >
                                            <div className="text-sm text-gray-500">
                                                ...
                                                {dropdownOpen === item.id && (
                                                    <div className="absolute right-8 z-30 mt-2 w-32 rounded-md border border-gray-200 bg-white shadow-lg">
                                                        <button
                                                            onMouseDown={(e) =>
                                                                e.stopPropagation()
                                                            }
                                                            ...
                                                            className="block w-full rounded-md px-4 py-2 text-center text-sm text-[#666666] hover:bg-[#CDCDCD]"
                                                        >
                                                            Delete
                                                        </button>
                                                    </div>
                        ...
                        <div className="flex w-full justify-center">
                            <div className="flex items-center space-x-2">
                                <button
                                    onClick={() =>
                                        handlePaginationClick(
                                            patient?.prev_page_url,
                                        )
                                    }
                                    disabled={!patient?.prev_page_url}
                                    className={`flex items-center rounded px-2 py-1 ${!patient?.prev_page_url ? "cursor-not-allowed text-gray-400" : "text-blue-600 hover:bg-blue-100"}`}
                                >
                                ...
                                <span className="custom-text-color flex items-center px-1">
                                    Page {patient?.current_page || 1} of{" "}
                                    {patient?.last_page || 1}
                                </span>
                                <button
                                    onClick={() =>
                                    ...
                                    disabled={!patient?.next_page_url}
                                    className={`flex items-center rounded px-2 py-1 ${!patient?.next_page_url ? "cursor-not-allowed text-gray-400" : "text-blue-600 hover:bg-blue-100"}`}
                                >
                                    <div className="custom-text-color flex items-center">
                                        <span className="mx-1">|</span>
                                        <span>Next</span>
