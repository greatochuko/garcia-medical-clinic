...
export default function PatientVisitForm(props, appointment) {
  ...
  const [setPatientCertificateAck, set_patient_certificate_ack] = useState(false);
  ...
  const inputNotInList =
    inputValue.trim() !== '' &&
    !plans.some((item) => item.name.toLowerCase() === inputValue.toLowerCase());
  ...
  const get_patient_certificate_ack = () => {
    ...
    axios.get(`/medical-certificate/patient/info/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
  ...
  const get_patient_medical_record = () => {
    axios.get(`/patient/medical-records/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
  ...
  const setmodifydone = () => {
    ...
    setdisabled(true);
    try {
      axios.put(`/patientvisitform/${patient.patient_id}/${this_appointment_id}`, {
  ...
  const get_patient_diagnosis = () => {
    axios.get(`/patientdiagnosis/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        if (response.data) {
  ...
  const get_chief_complaint = () => {
    axios.get(`/patient/chiefcomplaint/get/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        if (response.data) {
  ...
  const get_patient_notes = () => {
    axios.get(`/patient/notes/get/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        if (response.data) {
  ...
  const get_physical_exam = () => {
    axios.get(`/patient/physicalexam/get/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        if (response.data) {
  ...
  const fetchPrescriptions_list = () => {
    axios.get(`/patient/prescription/get/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        if (response.data && response.data.data) {
          const prescriptions = response.data.data;
          setpatientprescription(prescriptions);
  ...
  const get_patient_plan_list = () => {
    axios.get(`/patient/planlist/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        const data = response.data;
  ...
  const fetchpatientvisitformstatus = () => {
    axios.get(`/patientvisitform/${patient.patient_id}/${this_appointment_id}`)
      .then((response) => {
        const data = response.data;
  ...
  const fetchLabTestList = () => {
    axios.get(route('laboratory.options'))
      .then(response => {
  ...
  const fetchLabTests = () => {
    ...
    axios.get(route('laboratory.patient.tests', { patientId: patient.patient_id, app_id: this_appointment_id }))
      .then(response => {
  ...
  const handleAddTest = () => {
    ...
    setLoading(true);
    axios.post(route('laboratory.patient.store'), {
      patient_id: patient.patient_id,
  ...
  const fetchLabRequests = async () => {
    try {
      const response = await axios.get(`/laboratory-requests/${patient.patient_id}/${this_appointment_id}`);
  ...
  const handleSaveAndFinishLater = async () => {
    try {
      ...
      alert('Error: ' + (error.response?.data?.message || 'Failed to save document'));
    }
  };
  ...
  const handleSignOffAndClose = async () => {
    try {
      ...
      alert('Error: ' + (error.response?.data?.message || 'Failed to complete document'));
    }
  };
  ...
  const handleMedicalCertificate = async () => {
    try {
      const response = await fetch(`/medical-certificate/${patient.patient_id}/${this_appointment_id}`);
      ...
      window.open(`/medical-certificate/${patient.patient_id}/${this_appointment_id}`, '_blank');
    }
    catch (error) {
  ...
  /> */}
      ...
      >
        ...
        <VitalSignsModal isOpen={isVitalModalOpen} setIsOpen={setIsVitalModalOpen} patient={patientdata} onClose={() => {
        ...
        <div className="py-3">
          <div className="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div className="rounded-lg mb-6 ">
              <div className="relative flex items-center justify-center pb-0 mb-4">
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M2.34941 0.555908C1.8562 0.555908 1.38319 0.748823 1.03443 1.09221C0.685675 1.4356 0.489746 1.90134 0.489746 2.38697V17.0354C0.489746 17.5211 0.685675 17.9868 1.03443 18.3302C1.38319 18.6736 1.8562 18.8665 2.34941 18.8665H13.5074C14.0006 18.8665 14.4737 18.6736 14.8224 18.3302C15.1712 17.9868 15.3671 17.5211 15.3671 17.0354V4.59705C15.367 4.11147 15.171 3.64581 14.8222 3.3025L12.5776 1.09241C12.2289 0.748994 11.756 0.556012 11.2628 0.555908H2.34941ZM11.8691 8.60616C11.9579 8.5217 12.0287 8.42068 12.0774 8.30898C12.1262 8.19728 12.1518 8.07715 12.1529 7.95558C12.154 7.83402 12.1304 7.71346 12.0837 7.60095C12.0369 7.48843 11.9679 7.38621 11.8806 7.30025C11.7933 7.21429 11.6895 7.14631 11.5752 7.10027C11.4609 7.05424 11.3385 7.03108 11.215 7.03213C11.0915 7.03319 10.9695 7.05844 10.8561 7.10643C10.7426 7.15441 10.64 7.22415 10.5543 7.3116L6.60992 11.1962L5.2942 9.90071C5.11883 9.73394 4.88395 9.64166 4.64015 9.64375C4.39635 9.64583 4.16314 9.74212 3.99074 9.91187C3.81834 10.0816 3.72055 10.3112 3.71843 10.5513C3.71632 10.7913 3.81004 11.0226 3.97941 11.1953L5.88557 13.073C5.98056 13.1666 6.09333 13.2408 6.21746 13.2915C6.34158 13.3421 6.47463 13.3682 6.60899 13.3682C6.74334 13.3682 6.87639 13.3421 7.00051 13.2915C7.12464 13.2408 7.23741 13.1666 7.3324 13.073L11.8691 8.60616Z" fill="white"/>
       ...
       &nbsp;     Close and Checkout
              ...
              <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div class="lg:col-span-10 bg-white p-8">
                  <div className={`text-lg font-bold head-text uppercase absolute left-1/2 transform -translate-x-1/2   ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>
                    <b> Patient Visit Form </b>
                  </div>
                  <span class={`text-lg font-700 mt-5 ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`} >
                    {patient.first_name} {patient.last_name} <span class=" font-medium text-sm font-light-grey ">{patient.age}, {patient.gender}</span>
                  </span>
                  <div>
                    <div class="grid grid-cols-1 md:grid-cols-2 border-box lg:grid-cols-5 text-sm text-gray-600 mt-2 gap-2 ml-1 ">
                      <div class="font-light-grey"><span class={`font-semibold  ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Patient ID:</span> <br /> {patient.patient_id}</div>
                      <div class="font-light-grey border-box"><span class={`font-semibold ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Date of Birth:</span> <br /> {new Date(patient.dob).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'})}</div>
                      <div class="font-light-grey border-box"><span class={`font-semibold ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Mobile Number:</span> <br /> {patient.phone}</div>
                      <div class="font-light-grey border-box"><span class={`font-semibold ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Home Address:</span> <br /> {patient.address}</div>
                    </div>
                    <div class="mt-10 flex flex-col items-start ml-1 border-box">
                      <span class="flex items-center gap-1">
                        <p class={`text-sm font-semibold text-[#429ABF] ${getdisabled ? 'text-[#666666]' : 'text-[#429ABF]'}`}>Medical History</p>
                        <button onClick={() => setModalVisible(true)} disabled={getdisabled} className = {`  ${getdisabled ? 'text-[#666666]' : 'text-[#429ABF]'} `}>
                      ...
                      <span>
                        <p className="text-sm text-[#696969] font-extrabold">
...
<span>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 text-sm text-gray-600 mt-1 font-light-grey">
          ...
          <div><span class="font-semibold ">Weight:</span> <br /> 68 kg</div>
                ...
                <div class="flex flex-col gap-2 lg:col-span-2  p-3 text-center bg-white">
                  <p class="text-green-700 text-xs text-[12px] font-black">SAVE BUTTONS</p>


                  {patientvisitform === 'open' && (
                    <div className="flex flex-col gap-2 mt-4">
                      <button
                        className="w-[10rem] p-1 text-sm text-white rounded bg-green-700 hover:bg-green-800 mx-auto text-left flex flex-row"
                        onClick={() => {
                          setSelectedAction('sign_off');
                      ...
                      >
                        <svg width="16" height="19" className="mt-1" viewBox="0 0 16 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                        ...
                        &nbsp;<span className="text-[0.7rem]">Sign Off & Close Form</span>
                      </button>

                      <button
                        className="w-[10rem] p-1 text-sm rounded bg-white hover:bg-[#FFF9E6] text-green-700 border-green-700 border-2 mx-auto text-left flex flex-row"
                        onClick={() => {
                          setSelectedAction('save_and_finish');
                      ...
                      >
                        ...
                        &nbsp;<span className="text-[0.7rem]" >Save & Finish Later</span>
                      </button>
                    </div>
                  )}
                  ...
                  {patientvisitform === 'closed' && (
                    <div className="flex flex-col gap-2 mt-4">
                      <button
                        className="w-[10rem] p-1 text-sm text-white rounded bg-green-700 mx-auto text-left flex flex-row"
                        onClick={() => {
                      ...
                      >
                        ...
                        &nbsp;<span className="text-[0.7rem]">Modify Record</span>
                      </button>
                    </div>
                  )}
                  ...
                  >
                    ...
                    <span className="text-[0.7rem]">Laboratory Request</span>
                  </a>


                  <button class="w-[10rem] p-1 px-2 text-sm text-white rounded print-buttons  mx-auto flex flex-row" onClick={handleMedicalCertificate}>
                    <svg width="20" class="mr-2" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M15.5002 4.18498H4.50016V0.518311H15.5002V4.18498ZM15.5002 9.22664C15.7599 9.22664 15.9777 9.13864 16.1537 8.96264C16.3297 8.78664 16.4174 8.56909 16.4168 8.30998C16.4162 8.05087 16.3282 7.83331 16.1528 7.65731C15.9774 7.48131 15.7599 7.39331 15.5002 7.39331C15.2404 7.39331 15.0229 7.48131 14.8475 7.65731C14.6721 7.83331 14.5841 8.05087 14.5835 8.30998C14.5829 8.56909 14.6709 8.78695 14.8475 8.96356C15.0241 9.14017 15.2417 9.22787 15.5002 9.22664ZM13.6668 15.185V11.5183H6.3335V15.185H13.6668ZM15.5002 17.0183H4.50016V13.3516H0.833496V7.85164C0.833496 7.07248 1.10086 6.41951 1.63558 5.89273C2.1703 5.36595 2.81961 5.10225 3.5835 5.10164H16.4168C17.196 5.10164 17.8493 5.36534 18.3767 5.89273C18.9041 6.42012 19.1674 7.07309 19.1668 7.85164V13.3516H15.5002V17.0183Z" fill="white" />
                    </svg>&nbsp;<span class="text-[0.7rem]">Medical Certificate</span>
                  </button>
                </div>
              </div>

              <div style={{ backgroundColor: '#F1F2F5' }} class="my-10 p-10 rounded">

                <div className="bg-white mb-5 p-5 shadow-[0px_4px_4px_0px_#00000040]" >
                  <div className={`text-sm py-1 flex justify-between ${getdisabled ? 'text-[#666666]' : 'text-[#FF8000]'}`}>- {new Date(patient.updated_at).toLocaleString('en-US', {
                    month: 'long',
                    day: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
                    minute: '2-digit',
                    hour12: true
                  })}<span className={`${getdisabled ? 'text-[#666666]' : 'text-[#FF8000]'}`}>Visit Form ID #{this_appointment_id}</span></div>
                  <hr />
                  <div class="mt-4  flex flex-col items-start ml-1">
                    <span class="flex items-center gap-1">
                      <p class={`text-sm font-semibold ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Vital Signs and Measurements</p>
                      <button onClick={() => openVitalForm(patient)} disabled={getdisabled} className = {`  ${getdisabled ? 'text-[#666666]' : 'text-[#429ABF]'} `}>
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                          <path d="M8 15C4.14 15 1 11.86 1 8C1 4.14 4.14 1 8 1C11.86 1 15 4.14 15 8C15 11.86 11.86 15 8 15ZM8 2C4.69 2 2 4.69 2 8C2 11.31 4.69 14 8 14C11.31 14 14 11.31 14 8C14 4.69 11.31 2 8 2Z" />
                          <path d="M8 11.5C7.72 11.5 7.5 11.28 7.5 11V5C7.5 4.72 7.72 4.5 8 4.5C8.28 4.5 8.5 4.72 8.5 5V11C8.5 11.28 8.28 11.5 8 11.5Z" />
                          <path d="M11 8.5H5C4.72 8.5 4.5 8.28 4.5 8C4.5 7.72 4.72 7.5 5 7.5H11C11.28 7.5 11.5 7.72 11.5 8C11.5 8.28 11.28 8.5 11 8.5Z" />
  ...
  <div><span class="font-semibold">Weight</span><br />68 kg</div>
                    ...
                    <table className="w-[90%] text-sm text-gray-600 mt-1 m-3 table-fixed">
                      <thead>
                        <tr>
                          <th className="w-1/6 text-left font-semibold">Blood Pressure</th>
                          <th className="w-1/6 text-left font-semibold">Heart Rate</th>
                          <th className="w-1/6 text-left font-semibold">O2 Saturation</th>
                          <th className="w-1/6 text-left font-semibold">Temperature</th>
                          <th className="w-1/6 text-left font-semibold">Height</th>
                          <th className="w-1/6 text-left font-semibold">Weight</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          <td className="w-1/6">
                            {data.blood_systolic_pressure}/{data.blood_diastolic_pressure}
                          </td>
                          <td className="w-1/6"> {data?.heart_rate}</td>
                          <td className="w-1/6">{data?.o2saturation}%</td>
                          <td className="w-1/6">{data?.temperature}°C</td>
                          <td className="w-1/6">{data?.height_ft}'{data.height_in}"</td>
                          <td className="w-1/6">{data?.weight} kg</td>
                        </tr>
                      </tbody>
                    </table>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                  ...
                  <div className="md:col-span-3 flex flex-col gap-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div className="bg-white p-2 rounded shadow-[0px_4px_4px_0px_#00000040]">
                        ...
                        <div className="p-4">
                          <div className="card-screen">
                            <ul>
                              {ChiefComplaints.map((item, index) => (
                                <li key={index}>  &gt;
                                  {item.chief_complaint}</li>
                              ))}
                            </ul>
                          </div>
                          <textarea
                            className={`w-full rounded p-2 mt-2 resize-none ${getdisabled ? 'bg-[#CDCDCD] border-black text-white' : 'border border-[#429ABF] focus:outline-none focus:border-[#A7D3E8] focus:ring-0'}`}
                            rows="2"
                            onChange={(e) => setaddchiefcomplaint(e.target.value)}
                            value={addchiefcomplaint}
                            disabled={getdisabled} 
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && !e.shiftKey && !getdisabled) {
                          ...
                          <button className={`mt-2 text-sm text-white  px-3 py-1 rounded w-full ${getdisabled ? 'bg-[#666666] border border-black text-white' : 'print-buttons save-button'}`}
                            onClick={AddChiefComplaint} >+ Add Note</button>
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-2 rounded shadow-[0px_4px_4px_0px_#00000040]">
                        <div class="flex justify-between">
                          <span className={`font-600 m-2 text-[14px] font-semibold uppercase ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`} >Physical Exam</span>
                          <button className="p-1" onClick={openPhysicalExamModal} disabled={getdisabled}>
                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M11.8143 3.63428C12.0267 3.63451 12.231 3.71422 12.3854 3.85713C12.5399 4.00004 12.6328 4.19535 12.6453 4.40317C12.6577 4.61099 12.5887 4.81562 12.4524 4.97526C12.3161 5.13489 12.1227 5.23749 11.9118 5.26208L11.8143 5.26779H5.14762V16.7024H16.8143V10.1683C16.8145 9.96017 16.8959 9.75994 17.0417 9.60856C17.1875 9.45719 17.3868 9.36609 17.5988 9.35389C17.8108 9.34169 18.0196 9.40931 18.1825 9.54292C18.3454 9.67653 18.45 9.86606 18.4751 10.0728L18.481 10.1683V16.7024C18.4811 17.1145 18.3223 17.5115 18.0364 17.8136C17.7505 18.1158 17.3586 18.3009 16.9393 18.3318L16.8143 18.3359H5.14762C4.72714 18.3361 4.32215 18.1804 4.01383 17.9002C3.70552 17.62 3.51666 17.2359 3.48512 16.8249L3.48096 16.7024V5.26779C3.48082 4.85568 3.63963 4.45874 3.92554 4.15655C4.21145 3.85437 4.60333 3.66927 5.02262 3.63836L5.14762 3.63428H11.8143ZM17.0168 3.91443C17.1668 3.76794 17.368 3.6829 17.5796 3.67656C17.7912 3.67023 17.9974 3.74308 18.1562 3.88032C18.315 4.01757 18.4145 4.20891 18.4346 4.41549C18.4547 4.62206 18.3938 4.82838 18.2643 4.99255L18.1951 5.07014L9.94512 13.1552C9.79516 13.3017 9.59393 13.3868 9.3823 13.3931C9.17068 13.3994 8.96453 13.3266 8.80573 13.1893C8.64692 13.0521 8.54737 12.8607 8.52729 12.6542C8.50721 12.4476 8.56811 12.2413 8.69762 12.0771L8.76679 12.0003L17.0168 3.91443Z" fill="#666666" />
                            </svg>
                          </button>
                        ...
                        <div className="p-4">
                          <div className="card-screen">
                            <ul>
                              {PhysicalExam.map((item, index) => (
                                <li key={index}>  &gt;
                                  {item.physical_exam}</li>
                              ))}
                            </ul>
                          </div>
                          <textarea className={`w-full rounded p-2 resize-none  mt-2 ${getdisabled ? 'bg-[#CDCDCD] border border-black text-white' : 'border border-[#429ABF] focus:outline-none focus:border-[#A7D3E8] focus:ring-0'}`} rows="2" value={addphysicalexam} onChange={(e) => setaddphysicalexam(e.target.value)}
                            disabled={getdisabled}  onKeyDown={(e) => {
                              if (e.key === 'Enter' && !e.shiftKey && !getdisabled) {
                                e.preventDefault(); // Prevents newline from being added
                                AddPhysicalExam();
                              }
                            }}/>
                          <button className={`mt-2 text-sm text-white px-3 py-1 rounded w-full ${getdisabled ? 'bg-[#666666] border border-black text-white' : 'print-buttons save-button'}`} onClick={AddPhysicalExam}>+ Add Note</button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5 ">
                      {/* Plan */}
                      <div className="bg-white p-4 rounded shadow-[0px_4px_4px_0px_#00000040] flex flex-col justify-between min-h-[300px]">
                        <div class="mb-10"> {/* Content block */}
                          <div className="flex justify-between">
                            <h2 className={`font-semibold  mb-2 ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>PLAN</h2>
                            <div class="flex justify-between">
                              <button
                                className={`text-white md:text-[12px] mx-2 lg:text-[12px] lg:w-[129px] rounded text-center plan-buttons-1 ${getdisabled ? 'bg-[#666666] border border-black text-white' : 'bg-[#FF8000] hover:bg-[#E67300]'}`}
                                style={{ Width: '79px', height: '1.5rem' }} onClick={() => setIsMedicalCertificateOpen(true)} disabled={getdisabled}
                              >
                                Medical Certificate
                              </button>
                              <button
                                className={`text-white lg:text-[12px] lg:w-[90px]  rounded text-center px-2  plan-buttons-2 ${getdisabled ? 'bg-[#666666] ' : 'bg-[#FF8000] hover:bg-[#E67300]'}`} disabled={getdisabled}
                                style={{ height: '1.5rem' }}
                              ...
                              <button class="" onClick={OpenPatientPlanModal} disabled={getdisabled}>
                          ...
                          <div className="text-sm card-screen">
                            <ul className='font-normal font-poopins text-[14px] text-[black]'>
                              {patientplans?.map((item, index) => (
                                <li key={index}>
                                 &gt; {item.plan}
                                </li>
                              ))}
                            ...
                            {labRequests.length > 0 && (
                              <div className="mt-2 font-normal font-poopins text-[14px] text-[#429ABF]">
                                <span className={`${getdisabled ? 'text-black' : ''}`}>For:</span>
                                <div className={`block ${getdisabled ? 'text-black' : ''}`}>
                                &gt;  {labRequests.join(', ')}
                                </div>
                              </div>
                            )}
                            {setPatientCertificateAck && (
                              <div className={`mt-2 font-normal font-poopins text-[14px]  ${getdisabled ? 'text-black' : 'text-[#429ABF]'}`}>
                        ...
                        <div className="relative w-full">
                          <input
                            ...
                            className="w-full rounded border border-[#666666] bg-white text-sm px-3 py-2 pr-8 focus:outline-none"
                          ...
                          {showDropdown && (
                            <ul ref={wrapperRef} className="absolute z-[999999] pointer-events-auto mt-1 max-h-[130px] w-full overflow-auto rounded border bg-white text-sm shadow-md">
                              ...
                              {inputNotInList && (
                                <li
                                  ...
                                  className="cursor-pointer px-3 py-2 hover:bg-gray-100 text-blue-600"
                                >
                                  {inputValue}
                                </li>
                              )}
                      ...
                      {/* Diagnosis */}
                      <div className="bg-white p-4 rounded shadow-[0px_4px_4px_0px_#00000040]">
                        <div class="flex justify-between">
                          <h2 className={`font-semibold mb-2 uppercase ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>DIAGNOSIS</h2>
                          <button className="" onClick={OpenPatientDiagnosisModal} disabled={getdisabled}>
                            <svg width="25" height="19" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M11.8143 3.63428C12.0267 3.63451 12.231 3.71422 12.3854 3.85713C12.5399 4.00004 12.6328 4.19535 12.6453 4.40317C12.6577 4.61099 12.5887 4.81562 12.4524 4.97526C12.3161 5.13489 12.1227 5.23749 11.9118 5.26208L11.8143 5.26779H5.14762V16.7024H16.8143V10.1683C16.8145 9.96017 16.8959 9.75994 17.0417 9.60856C17.1875 9.45719 17.3868 9.36609 17.5988 9.35389C17.8108 9.34169 18.0196 9.40931 18.1825 9.54292C18.3454 9.67653 18.45 9.86606 18.4751 10.0728L18.481 10.1683V16.7024C18.4811 17.1145 18.3223 17.5115 18.0364 17.8136C17.7505 18.1158 17.3586 18.3009 16.9393 18.3318L16.8143 18.3359H5.14762C4.72714 18.3361 4.32215 18.1804 4.01383 17.9002C3.70552 17.62 3.51666 17.2359 3.48512 16.8249L3.48096 16.7024V5.26779C3.48082 4.85568 3.63963 4.45874 3.92554 4.15655C4.21145 3.85437 4.60333 3.66927 5.02262 3.63836L5.14762 3.63428H11.8143ZM17.0168 3.91443C17.1668 3.76794 17.368 3.6829 17.5796 3.67656C17.7912 3.67023 17.9974 3.74308 18.1562 3.88032C18.315 4.01757 18.4145 4.20891 18.4346 4.41549C18.4547 4.62206 18.3938 4.82838 18.2643 4.99255L18.1951 5.07014L9.94512 13.1552C9.79516 13.3017 9.59393 13.3868 9.3823 13.3931C9.17068 13.3994 8.96453 13.3266 8.80573 13.1893C8.64692 13.0521 8.54737 12.8607 8.52729 12.6542C8.50721 12.4476 8.56811 12.2413 8.69762 12.0771L8.76679 12.0003L17.0168 3.91443Z" fill="#666666" />
                            </svg>
                          </button>
                        ...
                        <div class="card-screen">
                          <ul>
                            {patientdiagnosislist?.map((item, index) => (
                              <li key={index}>  &gt;
                                {item.diagnosis}</li>
                            ))}
                          </ul>
                        </div>
                        <textarea className={`w-full border resize-none rounded p-2 ${getdisabled ? 'bg-[#CDCDCD] border border-black text-white' : 'border border-[#429ABF] focus:outline-none focus:border-[#A7D3E8] focus:ring-0'}`} defaultValue="" value={patientdiagnosis} onChange={(e) => setaddpatientdiagnosis(e.target.value)} disabled={getdisabled} 
                           onKeyDown={(e) => {
                            if (e.key === 'Enter' && !e.shiftKey && !getdisabled) {
                              e.preventDefault(); // Prevents newline from being added
                              AddPatientdiagnosis();
                        ...
                        <button className={`mt-2 text-sm text-white w-full px-3 py-1 rounded ${getdisabled ? 'bg-[#666666] border border-black text-white' : 'print-buttons save-button'}`} onClick={AddPatientdiagnosis}>+ Add Note</button>
                      </div>

                      {/* Notes */}
                      <div className="bg-white p-4 rounded shadow-[0px_4px_4px_0px_#00000040]">
                        <div class="flex justify-between">
                          <h2 className={`font-semibold mb-2 uppercase ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>NOTES</h2>
                          <button className="" onClick={OpenPatientModal} disabled={getdisabled}>
                            <svg width="25" height="19" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M11.8143 3.63428C12.0267 3.63451 12.231 3.71422 12.3854 3.85713C12.5399 4.00004 12.6328 4.19535 12.6453 4.40317C12.6577 4.61099 12.5887 4.81562 12.4524 4.97526C12.3161 5.13489 12.1227 5.23749 11.9118 5.26208L11.8143 5.26779H5.14762V16.7024H16.8143V10.1683C16.8145 9.96017 16.8959 9.75994 17.0417 9.60856C17.1875 9.45719 17.3868 9.36609 17.5988 9.35389C17.8108 9.34169 18.0196 9.40931 18.1825 9.54292C18.3454 9.67653 18.45 9.86606 18.4751 10.0728L18.481 10.1683V16.7024C18.4811 17.1145 18.3223 17.5115 18.0364 17.8136C17.7505 18.1158 17.3586 18.3009 16.9393 18.3318L16.8143 18.3359H5.14762C4.72714 18.3361 4.32215 18.1804 4.01383 17.9002C3.70552 17.62 3.51666 17.2359 3.48512 16.8249L3.48096 16.7024V5.26779C3.48082 4.85568 3.63963 4.45874 3.92554 4.15655C4.21145 3.85437 4.60333 3.66927 5.02262 3.63836L5.14762 3.63428H11.8143ZM17.0168 3.91443C17.1668 3.76794 17.368 3.6829 17.5796 3.67656C17.7912 3.67023 17.9974 3.74308 18.1562 3.88032C18.315 4.01757 18.4145 4.20891 18.4346 4.41549C18.4547 4.62206 18.3938 4.82838 18.2643 4.99255L18.1951 5.07014L9.94512 13.1552C9.79516 13.3017 9.59393 13.3868 9.3823 13.3931C9.17068 13.3994 8.96453 13.3266 8.80573 13.1893C8.64692 13.0521 8.54737 12.8607 8.52729 12.6542C8.50721 12.4476 8.56811 12.2413 8.69762 12.0771L8.76679 12.0003L17.0168 3.91443Z" fill="#666666" />
                            </svg>
                          </button>
                        ...
                        <div class="card-screen">
                          <ul>
                            {PatientNotes.map((item, index) => (
                              <li key={index}> &gt;
                                {item.note}</li>
                            ))}
                          </ul>
                        </div>
                        <textarea className={`w-full border resize-none rounded p-2 ${getdisabled ? 'bg-[#CDCDCD] border border-black text-white' : 'border border-[#429ABF] focus:outline-none focus:border-[#A7D3E8] focus:ring-0'}`} defaultValue="" value={addpatientnotes} onChange={(e) => setaddpatientnotes(e.target.value)} disabled={getdisabled} 
                           onKeyDown={(e) => {
                            if (e.key === 'Enter' && !e.shiftKey && !getdisabled) {
                              e.preventDefault(); // Prevents newline from being added
                              AddPatientnotes();
                        ...
                        <button className={`mt-2 text-sm text-white w-full  px-3 py-1 rounded ${getdisabled ? 'bg-[#666666] border border-black text-white' : 'print-buttons save-button'}`} onClick={AddPatientnotes}>+ Add Note</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-2 rounded shadow-[0px_4px_4px_0px_#00000040]">
                    <div className="flex justify-between">
                      <h2 className={`font-semibold text-[14px] m-2 uppercase ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Prescription</h2>
                      <div className="flex flex-row">
                        <div className="justify-between flex">
                          <button
                            className={`w-[7.5rem] text-center h-[26px] m-auto text-white rounded   ${getdisabled ? 'bg-[#666666]' : 'bg-[#FF8000] hover:bg-[#E67300]'}`} disabled={getdisabled}
                            onClick={openAddPrescription}
                            style={{ backgroundColor: '' }}
                          >
                            <span className="text-[0.8rem]">Add Prescription</span>
                          </button>
                          <button className="p-1" onClick={openEditPrescription} disabled={getdisabled}>
                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M11.8143 3.63428C12.0267 3.63451 12.231 3.71422 12.3854 3.85713C12.5399 4.00004 12.6328 4.19535 12.6453 4.40317C12.6577 4.61099 12.5887 4.81562 12.4524 4.97526C12.3161 5.13489 12.1227 5.23749 11.9118 5.26208L11.8143 5.26779H5.14762V16.7024H16.8143V10.1683C16.8145 9.96017 16.8959 9.75994 17.0417 9.60856C17.1875 9.45719 17.3868 9.36609 17.5988 9.35389C17.8108 9.34169 18.0196 9.40931 18.1825 9.54292C18.3454 9.67653 18.45 9.86606 18.4751 10.0728L18.481 10.1683V16.7024C18.4811 17.1145 18.3223 17.5115 18.0364 17.8136C17.7505 18.1158 17.3586 18.3009 16.9393 18.3318L16.8143 18.3359H5.14762C4.72714 18.3361 4.32215 18.1804 4.01383 17.9002C3.70552 17.62 3.51666 17.2359 3.48512 16.8249L3.48096 16.7024V5.26779C3.48082 4.85568 3.63963 4.45874 3.92554 4.15655C4.21145 3.85437 4.60333 3.66927 5.02262 3.63836L5.14762 3.63428H11.8143ZM17.0168 3.91443C17.1668 3.76794 17.368 3.6829 17.5796 3.67656C17.7912 3.67023 17.9974 3.74308 18.1562 3.88032C18.315 4.01757 18.4145 4.20891 18.4346 4.41549C18.4547 4.62206 18.3938 4.82838 18.2643 4.99255L18.1951 5.07014L9.94512 13.1552C9.79516 13.3017 9.59393 13.3868 9.3823 13.3931C9.17068 13.3994 8.96453 13.3266 8.80573 13.1893C8.64692 13.0521 8.54737 12.8607 8.52729 12.6542C8.50721 12.4476 8.56811 12.2413 8.69762 12.0771L8.76679 12.0003L17.0168 3.91443Z" fill="#666666" />
                            </svg>
                          </button>
                    ...
                    <div className="mt-5 max-h-[700px] overflow-auto pr-2">
                      <ul className={`text-sm text-gray-700 list-decimal pl-6 space-y-1  ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>
                        {patientprescription.map((item) => (
                          <li key={item.id}>
                            <div className="flex justify-between w-full mt-5">
                              <span>{item.medication}</span>
                              <span className={` ${getdisabled ? 'text-[#666666]' : 'text-[#FF8000]'}`} ># {item.amount}</span>
                            </div>
                            <div className="mt-1">
                              <span className="text-gray-500">
                                {item.dosage}&nbsp;{item.frequency}&nbsp;{item.days}
                             {item.duration && (
                  ...
                  <div className="bg-white p-4 rounded shadow mt-5 md:col-span-3 gap-2 shadow-[0px_4px_4px_0px_#00000040]">
                    <h2 className={`font-semibold ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>Laboratory Values</h2>
                    ...
                    <div className="flex gap-2 mb-4">
                      <div className="relative w-[272px]">
                        <input
                          className="border textboxs p-2 rounded w-full text-[#999999] text-[14px] font-normal font-poppins"
                          type="text"
                          value={searchTerm}
                          onChange={(e) => {
                        ...
                        {showList && (
                          <ul ref={wrapperRef} className="absolute z-[9999] bg-white border w-full max-h-48 overflow-y-auto mt-1 rounded shadow">
                            {filteredOptions?.length > 0 ? (
                              filteredOptions.map((test, index) => (
                                <li
                                  key={index}
                                  onClick={() => handleSelectw(test)}
                                  className="p-2 hover:bg-gray-200 cursor-pointer text-[#999999] text-[14px] font-normal font-poppins"
                                >
                                  {test}
                                </li>
                              ))
                              ...
                              <li
                                onClick={() => handleSelectw(searchTerm)}
                                className="p-2 hover:bg-gray-200 cursor-pointer text-blue-600"
                              >
                                {searchTerm}
                              </li>
                            )}
                      ...
                      <div className="relative w-[403px]">
                        <input
                          ...
                          className="textboxs pr-24 rounded w-full"
                          value={testResult}
                          onChange={(e) => setTestResult(e.target.value)}
                          disabled={getdisabled}
                        />
                        <button
                          className={`absolute right-0 top-0 bottom-0 px-8 text-[14px] font-normal font-poppins text-white px-3 rounded-r ${getdisabled ? 'bg-[#666666]' : 'print-buttons save-button '}`}
                    ...
                    <div className="relative border rounded max-h-[300px] overflow-hidden">
                      <div className="overflow-x-auto">
                        <div className="overflow-y-auto max-h-[300px]">
                          <table className="w-full text-sm table-auto">
                            <thead className="sticky top-0 bg-[#E9F9FF] text-[#666666] z-10 w-full">
                              <tr>         <th className="text-[#666666] text-[14px] font-normal font-poppins p-2 whitespace-nowrap sticky left-0 bg-[#E9F9FF] z-20 text-left">Test Name</th>
                                {testDates.map((date, index) => (
                                  <th key={index} className="p-2 whitespace-nowrap text-center text-[#666666] text-[14px] font-normal font-poppins" style={{ minWidth: '120px' }}>
                                    {index === 0 ? 'TODAY' : date}
                                  </th>
                                ))}
                                <th></th>
                            ...
                            <tbody>
                              {loading ? (
                                <tr>
                                  <td colSpan={testDates.length + 2} className="p-4 text-center text-[#666666] text-[14px] font-normal font-poppins">
                                    Loading...
                                  </td>
                                </tr>
                              ) : labTests.length === 0 ? (
                                ...
                                labTests.map((test, index) => (
                                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-[#F1F2F5]'}>
                                    <td className="p-2 whitespace-nowrap sticky left-0 bg-inherit text-left text-[#666666] text-[14px] font-normal font-poppins">{test.test_name}</td>
                                    {testDates.map((date, dateIndex) => (
                                      <td key={dateIndex} className="p-2 whitespace-nowrap text-center text-[#666666] text-[14px] font-normal font-poppins" style={{ minWidth: '120px' }}>
                                        {test.values[date] || '-'}
                                      </td>
                                    ))}
                                    <td className="p-2 whitespace-nowrap sticky right-0 bg-inherit text-center">
                                      <button onClick={() => openEditLabOpen(test)} disabled={getdisabled}>
                                        <svg width="25" height="25" className="m-auto cursor-pointer" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                          <path d="M11.8143 3.63428C12.0267 3.63451 12.231 3.71422 12.3854 3.85713C12.5399 4.00004 12.6328 4.19535 12.6453 4.40317C12.6577 4.61099 12.5887 4.81562 12.4524 4.97526C12.3161 5.13489 12.1227 5.23749 11.9118 5.26208L11.8143 5.26779H5.14762V16.7024H16.8143V10.1683C16.8145 9.96017 16.8959 9.75994 17.0417 9.60856C17.1875 9.45719 17.3868 9.36609 17.5988 9.35389C17.8108 9.34169 18.0196 9.40931 18.1825 9.54292C18.3454 9.67653 18.45 9.86606 18.4751 10.0728L18.481 10.1683V16.7024C18.4811 17.1145 18.3223 17.5115 18.0364 17.8136C17.7505 18.1158 17.3586 18.3009 16.9393 18.3318L16.8143 18.3359H5.14762C4.72714 18.3361 4.32215 18.1804 4.01383 17.9002C3.70552 17.62 3.51666 17.2359 3.48512 16.8249L3.48096 16.7024V5.26779C3.48082 4.85568 3.63963 4.45874 3.92554 4.15655C4.21145 3.85437 4.60333 3.66927 5.02262 3.63836L5.14762 3.63428H11.8143ZM17.0168 3.91443C17.1668 3.76794 17.368 3.6829 17.5796 3.67656C17.7912 3.67023 17.9974 3.74308 18.1562 3.88032C18.315 4.01757 18.4145 4.20891 18.4346 4.41549C18.4547 4.62206 18.3938 4.82838 18.2643 4.99255L18.1951 5.07014L9.94512 13.1552C9.79516 13.3017 9.59393 13.3868 9.3823 13.3931C9.17068 13.3994 8.96453 13.3266 8.80573 13.1893C8.64692 13.0521 8.54737 12.8607 8.52729 12.6542C8.50721 12.4476 8.56811 12.2413 8.69762 12.0771L8.76679 12.0003L17.0168 3.91443Z" fill="#666666" />
                                        </svg>
                                      </button>
                                    </td>
                  ...
                  <div className="bg-white p-4 overflow-y-auto rounded shadow mt-5 shadow-[0_4px_4px_0_#00000040]">
                    <h2 className="font-semibold text-gray-700 mb-4">Previous Records</h2>

                    <div className="relative pl-10 ">
                      {/* Vertical center line */}
                      <div className="absolute left-[28px]  top-0 bottom-0 w-[2px] bg-gray-300 z-0 "></div>

                      {/* Entry 1 - May 9, 2025 */}
                      {medicalrecords.map((record, index) => (
                        <div key={index} className="relative flex items-start mb-6 z-10">
                          <div className="absolute left-[-28px] top-0 w-8 h-8 flex items-center justify-center rounded-full bg-gray-200">
                            <svg width="24" height="24" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <rect width="32" height="32" rx="16" fill="#EAECF0" />
                              <g clipPath="url(#clip0_1040_900)">
                                <path d="M17.7267 9.72671C17.4734 9.47337 17.1334 9.33337 16.7801 9.33337H12.0001C11.2667 9.33337 10.6667 9.93337 10.6667 10.6667V21.3334C10.6667 22.0667 11.2601 22.6667 11.9934 22.6667H20.0001C20.7334 22.6667 21.3334 22.0667 21.3334 21.3334V13.8867C21.3334 13.5334 21.1934 13.1934 20.9401 12.9467L17.7267 9.72671ZM18.0001 20H14.0001C13.6334 20 13.3334 19.7 13.3334 19.3334C13.3334 18.9667 13.6334 18.6667 14.0001 18.6667H18.0001C18.3667 18.6667 18.6667 18.9667 18.6667 19.3334C18.6667 19.7 18.3667 20 18.0001 20ZM18.0001 17.3334H14.0001C13.6334 17.3334 13.3334 17.0334 13.3334 16.6667C13.3334 16.3 13.6334 16 14.0001 16H18.0001C18.3667 16 18.6667 16.3 18.6667 16.6667C18.6667 17.0334 18.3667 17.3334 18.0001 17.3334ZM16.6667 13.3334V10.3334L20.3334 14H17.3334C16.9667 14 16.6667 13.7 16.6667 13.3334Z" fill={`${getdisabled ? '#666666' : '#429ABF'}`} />
                              </g>
                            </svg>
                          </div>
                          <div className="flex flex-col">
                            <span className="ml-10 text-sm font-light-grey">{new Date(record.date).toLocaleDateString('en-US', {
                              month: 'long',
                              day: '2-digit',
                              year: 'numeric',
                            })}</span>
                            <span className={`ml-10 text-sm ${getdisabled ? 'text-[#666666]' : 'font-light-blue'}`}>{record.diagnosis}</span>
                            <span className={`ml-10 text-sm ${getdisabled ? 'text-[#666666]' : 'text-[#FF8000]'}`}>{record.doctor_first_name}</span>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {modifyaskModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white w-[90%] max-w-md rounded-lg shadow-lg p-6 relative">
              <button
                className="absolute top-2 right-3 text-gray-500 text-xl font-bold hover:text-red-500"
                onClick={() => setmodifyaskModalOpen(false)}
              >
                ×
              </button>

              <h3 className="text-lg font-semibold mb-2 text-gray-800 text-center">Modify Record?</h3>
              <hr className="mb-4" />

              <p className="text-sm text-gray-700 mb-6">
                This record has already been closed by <strong>Dr. {doctor_name}</strong>. Are you sure you want to modify it?
              </p>

              <div className="flex justify-end gap-3">
                <button
                  type="button"
                  className="px-4 py-2 bg-gray-300 text-gray-800 rounded cancel-button"
                  onClick={() => setmodifyaskModalOpen(false)}
                >
                  Cancel
                </button>
                <button
                  type="button"
                  className="px-4 py-2 bg-[#429ABF] text-white rounded save-button disabled:opacity-50"
                  disabled={processing}
                  onClick={() => changepatientvisitformstatus()}
                >
                  Continue
                </button>
              </div>
