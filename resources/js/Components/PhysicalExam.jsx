...
export default function PhysicalExamModal({ isOpen, onClose, patient , appointment_id }) {
  if (!isOpen) return null;

  const [PhysicalExamText, setPhysicalExamText] = useState('');
  const [originalPhysicalExam, setOriginalPhysicalExam] = useState([]);

  useEffect(() => {
    axios.get(`/patient/physicalexam/get/${patient.patient_id}/${appointment_id}`)
      .then((response) => {
        if (response.data) {
          const physicalexam = response.data.map(item => item.physical_exam);
          setPhysicalExamText(physicalexam.join('\n'));
          setOriginalPhysicalExam(physicalexam); // Store original complaints
        } else {
  ...
  const onSave = async () => {
    const current = PhysicalExamText
      .split('\n')
      .map(c => c.trim())
      .filter(c => c !== '');

    const original = originalPhysicalExam
      .map(c => c.trim())
      .filter(c => c !== '');
    ...
    try {
      for (const physicalexam of added) {
        await axios.post('/patient/physicalexam/add', {
      ...
      for (const physicalexam of deleted) {
        await axios.delete('/patient/physicalexam/delete', {
          data: {
            patient_id: patient.patient_id,
            physical_exam: physicalexam,
            appointment_id:appointment_id
