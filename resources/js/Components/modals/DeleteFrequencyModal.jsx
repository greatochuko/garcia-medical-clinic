...
export default function DeleteFrequencyModal({
    open,
    closeModal,
    frequencyToDelete,
}) {
    ...
    function handleDeleteFrequency() {
        deleteRequest(route("frequency-list.destroy", frequencyToDelete?.id), {
            onSuccess: () => {
                closeModal();
            },
            onError: () => {
                toast.error("An error occurred deleting frequency");
            },
            // preserveState: true,
            preserveScroll: true,
        });
        ...
        <ModalContainer closeModal={closeDeleteModal} open={open}>
            ...
            >
                <div className="flex items-center justify-between p-2 px-4 pr-2">
                    <h5 className="font-semibold">Delete Frequency</h5>
                    <button
