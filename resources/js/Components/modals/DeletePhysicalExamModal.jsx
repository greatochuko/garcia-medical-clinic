...
export default function DeletePhysicalExamModal({
    open,
    closeModal,
    examToDelete,
}) {
    ...
    function handleDeleteExam() {
        deleteRequest(route("physical-exam.destroy", examToDelete?.id), {
            onSuccess: () => {
                closeModal();
            },
            onError: () => {
                toast.error("An error occurred deleting physical exam");
            },
            // preserveState: true,
            preserveScroll: true,
        });
    ...
    return (
        <ModalContainer closeModal={closeDeleteModal} open={open}>
            <div
                ...
                className={`w-[90%] max-w-md divide-y-2 divide-accent-200 rounded-lg bg-white text-sm text-accent duration-200 ${open ? "" : "translate-y-2"}`}
            >
                <div className="flex items-center justify-between p-2 px-4 pr-2">
                    <h5 className="font-semibold">Delete Physical Exam</h5>
                    <button
                ...
                <div className="px-4 py-8">
                    <p>
                        Are you sure you want to delete the physical exam:{" "}
                        <span className="font-bold">{examToDelete?.name}</span>?
                    </p>
                </div>
