...
export default function DeletePlanModal({ open, closeModal, planToDelete }) {
    const { processing, delete: deleteRequest } = useForm();

    function handleDeletePlan() {
        deleteRequest(route("plan-list.destroy", planToDelete?.id), {
            onSuccess: () => {
                closeModal();
            },
            onError: () => {
                toast.error("An error occurred deleting plan");
            },
            // preserveState: true,
            preserveScroll: true,
        });
        ...
        <ModalContainer closeModal={closeDeleteModal} open={open}>
            ...
            >
                <div className="flex items-center justify-between p-2 px-4 pr-2">
                    <h5 className="font-semibold">Delete Plan</h5>
                    <button
