...
export default function DeletePatientModal({ open, closeModal, patientId }) {
    const { processing, delete: deleteRequest } = useForm();

    function handleDeletePatient() {
        deleteRequest(route("medicalrecords.delete", { id: patientId }), {
            onSuccess: () => {
                closeModal();
            },
            onError: () => {
        ...
        <ModalContainer closeModal={closeDeleteModal} open={open}>
            ...
            >
                <div className="flex items-center justify-between p-2 px-4 pr-2">
                    <h5 className="font-semibold">Delete Patient</h5>
                    <button
