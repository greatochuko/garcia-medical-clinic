...
export default function PatientDiagnosisModal({ isOpen, onClose, patient, appointment_id }) {
  ...
  useEffect(() => {
    axios.get(`/patientdiagnosis/${patient.patient_id}/${appointment_id}`)
      .then((response) => {
        if (response.data) {
          const patientplans = response.data.map(item => item.diagnosis);
  ...
  const onSave = async () => {
    ...
    try {
      for (const plans of added) {
        await axios.post('/patientdiagnosis', {
      ...
      for (const plans of deleted) {
        await axios.delete('/patientdiagnosis', {
          data: {
            patient_id: patient.patient_id,
            diagnosis: plans,
            appointment_id : appointment_id
